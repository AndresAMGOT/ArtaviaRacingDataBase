...
Requerimiento: AR-001
Fecha de Creacion: 14-07-24 DD-MM-YYYY
Enunciado de la Tabla: Tabla que almacena la relaci�n entre diagn�sticos y servicios
****************************************************************************************************************************************************************/  
...
Requerimiento: AR-001
Fecha de Creacion: 14-07-24 DD-MM-YYYY
Enunciado de la Tabla: Tabla que almacena la relaci�n entre diagn�sticos y productos
****************************************************************************************************************************************************************/  
...
Autor: Horacio Porras Mar�n
Requerimiento: AR-001
Fecha de Creacion: 21-07-24 DD-MM-YYYY
Enunciado de la vista: Muestra toda la informaci�n del producto, incluyendo la categoria por nombre.
****************************************************************************************************************************************************************/  
...
Requerimiento: AR-001
Fecha de Creacion: 21-07-24 DD-MM-YYYY
Enunciado de la vista: Muestra toda la informaci�n del servicio, incluyendo la categoria por nombre.
****************************************************************************************************************************************************************/  
...
Requerimiento: AR-001
Fecha de Creacion: 21-07-24 DD-MM-YYYY
Enunciado de la vista: Muestra la informaci�n de los vehiculos.
****************************************************************************************************************************************************************/  
...
Requerimiento: AR-001
Fecha de Creacion: 21-07-24 DD-MM-YYYY
Enunciado de la vista: Muestra la informaci�n de los puestos.
****************************************************************************************************************************************************************/  
...
CREATE OR REPLACE NONEDITIONABLE PROCEDURE USP_RegistrarCliente (
    ...
    -- Par�metros para TELEFONOPORUSUARIO
    ...
    -- Par�metros para DIRECCIONPORUSUARIO
    ...
    -- Par�metros para CREDENCIALESPORUSUARIO
...
BEGIN
    ...
    BEGIN
        ...
        -- Confirma la transacci�n si todos los inserts son exitosos
        COMMIT;
    EXCEPTION
...
BEGIN
    ...
    BEGIN
        ...
        SET
            ...
            CONTRASE�A = NVL(p_contrasena, CONTRASE�A),
            ESCONTRASE�ATEMPORAL = NVL(p_es_contrasena_temporal, ESCONTRASE�ATEMPORAL),
            EDITADOPOR = p_editado_por,
            HABILITADO = NVL(p_habilitado, HABILITADO),
            FECHACREACION = NVL(p_fecha_creacion, FECHACREACION)
        WHERE CREDENCIALID = p_credencial_id;
...
CREATE OR REPLACE NONEDITIONABLE PROCEDURE USP_EliminarCliente (
    -- Par�metro para identificar al cliente
    p_credencial_id IN VARCHAR2
) AS
BEGIN
    -- Inicia una transacci�n
...
CREATE OR REPLACE NONEDITIONABLE PROCEDURE USP_VerificarUsuario (
    p_correo_electronico IN VARCHAR2,
    p_contrasena         IN VARCHAR2, -- Contrase�a ya encriptada en SHA-512
    p_credencial_id      OUT VARCHAR2, -- CREDENCIALID del usuario si existe
    p_rol_id             OUT NUMBER     -- ROLID del usuario si existe
) AS
BEGIN
    -- Inicializa los par�metros de salida
    p_credencial_id := NULL;
    p_rol_id := NULL;
    
    -- Verifica si el usuario existe con el correo electr�nico y la contrase�a proporcionados
    BEGIN
        SELECT 
              c.CREDENCIALID,
              c.ROLID
        ...
        WHERE cr.CORREOELECTRONICO = p_correo_electronico
            AND cr.CONTRASE�A = p_contrasena;
        
        -- Si no se encuentra ning�n registro, p_credencial_id y p_rol_id ser�n NULL
    EXCEPTION
        -- Captura cualquier error y muestra un mensaje
        WHEN NO_DATA_FOUND THEN
            -- Si no se encuentra ning�n dato, p_credencial_id y p_rol_id permanecen NULL
            p_credencial_id := NULL;
            p_rol_id := NULL;
        WHEN OTHERS THEN
